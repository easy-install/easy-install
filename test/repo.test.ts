...
test("fromUrl", async () => {
  for (const [url, repo] of [
    ...
    ["https://github.com/ahaoboy/ansi2/releases/tag/v0.2.11", new Repo("ahaoboy", "ansi2", "v0.2.11")],
    ["https://github.com/ahaoboy/ansi2/releases/download/v0.2.11/ansi2-x86_64-unknown-linux-musl.tar.gz", new Repo("ahaoboy", "ansi2", "v0.2.11")],
    ["https://github.com/ahaoboy/ansi2/releases/download/v0.2.11/ansi2-x86_64-pc-windows-msvc.zip", new Repo("ahaoboy", "ansi2", "v0.2.11")],
    ["https://github.com/Ryubing/Ryujinx/releases/download/1.2.78/ryujinx-*.*.*-win_x64.zip", new Repo("Ryubing", "Ryujinx", "1.2.78")],
  ] as const) {
...
test("getArtifactUrls", async () => {
  const repo = Repo.fromUrl("https://github.com/ahaoboy/mujs-build/releases/tag/v0.0.4")!;
...
test("getManfiestUrl", async () => {
  for (const [url, json] of [
    ["https://github.com/axodotdev/cargo-dist/releases/tag/v0.25.1", "https://github.com/axodotdev/cargo-dist/releases/download/v0.25.1/dist-manifest.json"],
    ["https://github.com/ahaoboy/mujs-build/releases/tag/v0.0.2", "https://github.com/ahaoboy/mujs-build/releases/download/v0.0.2/dist-manifest.json"]
  ] as const) {
    const repo = Repo.fromUrl(url)!
...
test("getManfiest", async () => {
  ...
  expect(json.artifacts['cargo-dist-x86_64-apple-darwin.tar.xz'].name).toEqual("cargo-dist-x86_64-apple-darwin.tar.xz")
})